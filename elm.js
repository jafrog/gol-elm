...
Elm.Game.make = function (_elm) {
   ...
   var displayCell = F2(function (cell,
   size) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: $Basics.toFloat((cell.x + 1) * size)
                                    ,_1: $Basics.toFloat((0 - (cell.y + 1)) * size)})($Graphics$Collage.filled(function () {
         var _v0 = cell.state;
         switch (_v0.ctor)
         {case "Alive":
            return $Color.black;
         ...
         {case "_Tuple2":
            return function () {
                 var cellSize = function () {
                    var _v5 = $String.toInt(cellSizeString);
                    switch (_v5.ctor)
                    {case "Err":
                 ...
                 _L.fromArray([$Graphics$Collage.move({ctor: "_Tuple2"
                                                      ,_0: (0 - $Basics.toFloat(_v1._0)) / 2
                                                      ,_1: $Basics.toFloat(_v1._1) / 2})($Graphics$Collage.group(A2($List.map,
                 function (cell) {
                    return A2(displayCell,
                    cell,
                    cellSize);
                 },
                 A2($List.concatMap,
   ...
   var aliveCells = _L.fromArray([_L.fromArray([0
                                               ,0])
                                 ,_L.fromArray([0,1])
                                 ,_L.fromArray([1,0])]);
   var inRange = F3(function (x,
   y,
   dim2) {
      return _U.cmp(x,
      1) > -1 && (_U.cmp(y,
      1) > -1 && (_U.cmp(x,
      $Basics.sqrt($Basics.toFloat(dim2))) < 1 && _U.cmp(y,
      $Basics.sqrt($Basics.toFloat(dim2))) < 1));
   });
   var black = A3($Color.rgb,
   0,
   0,
   0);
   ...
   var cellCondition = function (neighbours) {
      return function () {
   ...
   var Dead = {ctor: "Dead"};
   var initCell = F2(function (row,
   col) {
      return function () {
         var _v12 = A2($List.any,
         function (_v13) {
            return function () {
               switch (_v13.ctor)
               {case "::":
                       ...
                       switch (_v13._1._1.ctor)
                         {case "[]":
                            return _U.eq(_v13._0,
                              row) && _U.eq(_v13._1._0,col);}
                         break;}
                    break;}
               _U.badCase($moduleName,
               "on line 56, column 24 to 44");
            }();
         },
         aliveCells);
         switch (_v12)
      ...
      return function () {
         ...
         {case "_Tuple2": return {_: {}
                                 ...
                                 function (row) {
                                    ...
                                    function (col) {
                                       return {_: {}
                                              ,state: A2(initCell,row,col)
                                              ,x: row
                                              ,y: col};
                                    },
                                    _L.range(0,_v19._1 - 1)));
                                 },
                                 _L.range(0,_v19._0 - 1)))
                                 ,cols: _v19._1
                                 ,rows: _v19._0};}
         _U.badCase($moduleName,
   ...
   var updateCell = F2(function (cell,
   game) {
      return _U.replace([["state"
                         ,function () {
                            var _v23 = cellCondition(A2(neighbours,
                            cell,
                            game));
                            switch (_v23.ctor)
                            {case "Overcrowding":
   ...
   var updateGame = F2(function (timestamp,
   game) {
...
Elm.Main.make = function (_elm) {
   ...
   var CellSize = function (a) {
      return {ctor: "CellSize"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   var updates = $Signal.channel(NoOp);
   var cellSizeSlider = function (cellSize) {
      return A2($Html.div,
      ...
      _L.fromArray([A2($Html.label,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Cell size")]))
                   ,A2($Html.input,
                   _L.fromArray([$Html$Attributes.type$("range")
                                ...
                                function ($) {
                                   return $Signal.send(updates)(CellSize($));
                                })]),
                   _L.fromArray([]))]));
   };
   var view = F3(function (state,
   _v0,
   action) {
      return function () {
         switch (_v0.ctor)
            ...
            return function () {
                 var cellSize = function () {
                    ...
                    {case "CellSize":
                       return action._0;
                       case "NoOp":
                       return $Basics.toString(A2($Basics.max,
                         _v0._0,
                         _v0._1) / 20 | 0);}
                    _U.badCase($moduleName,
                 ...
                 return A2($Html.div,
                 _L.fromArray([]),
                 _L.fromArray([cellSizeSlider(cellSize)
                              ,$Html.fromElement(A3($Game.display,
                              state,
                              {ctor: "_Tuple2"
                              ,_0: _v0._0
                              ,_1: _v0._1},
