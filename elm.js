...
Elm.Cell.make = function (_elm) {
   ...
   size) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: $Basics.toFloat(model.pos.j * size + model.pos.j)
                                    ,_1: $Basics.toFloat((0 - model.pos.i) * size - model.pos.i)})($Graphics$Collage.filled(function () {
         var _v0 = model.state;
         switch (_v0.ctor)
         {case "Alive":
            return $Color.black;
   ...
   var Stable = {ctor: "Stable"};
   var Reproduction = {ctor: "Reproduction"};
   var Overcrowding = {ctor: "Overcrowding"};
   var Underpopulation = {ctor: "Underpopulation"};
   var Alive = {ctor: "Alive"};
   var condition = function (neighbours) {
      return function () {
         var aliveNeighbours = $List.length(A2($List.filter,
         function (cell) {
            return function () {
               switch (cell.ctor)
               {case "Just":
                  return _U.eq(cell._0.state,
                    Alive);
         ...
         neighbours));
         return _U.cmp(aliveNeighbours,
         2) < 0 ? Underpopulation : _U.eq(aliveNeighbours,
         2) ? Stable : _U.cmp(aliveNeighbours,
         3) > 0 ? Overcrowding : _U.eq(aliveNeighbours,
         3) ? Reproduction : _U.badIf($moduleName,
   ...
   var update = F2(function (neighbours,
   model) {
      return _U.replace([["state"
                         ,function () {
                            var _v3 = condition(neighbours);
                            switch (_v3.ctor)
                            {case "Overcrowding":
                               return Dead;
                               case "Reproduction":
   ...
   var flip = function (cell) {
      return function () {
         ...
         {case "Just":
            return _U.eq(cell._0.state,
              Dead) ? $Maybe.Just(_U.replace([["state"
                                              ,Alive]],
              cell._0)) : _U.eq(cell._0.state,
              Alive) ? $Maybe.Just(_U.replace([["state"
                                               ,Dead]],
              cell._0)) : _U.badIf($moduleName,
...
Elm.Controls.make = function (_elm) {
   ...
   address) {
      ...
      _L.fromArray([A2($Html$Events.onClick,
                   address,
                   action)
                   ,$Html$Attributes.href(_U.eq(action,
                   "share") ? "#shareDialog" : "#")]),
...
Elm.Game.make = function (_elm) {
   ...
   var parsePair = function (pair) {
      return function () {
         var _v0 = A2($List.map,
         function ($int) {
            return $String.toInt($int);
         },
         A2($String.split,",",pair));
         switch (_v0.ctor)
         {case "::": switch (_v0._0.ctor)
              {case "Err":
                 return $Maybe.Nothing;
   ...
   var parseStateParams = function (stateParams) {
      return A2($List.map,
      parsePair,
      A2($String.split,
      ";",
   ...
   j,
   game) {
      return i * game.rows + j;
   });
   var getCell = F3(function (game,
   i,
   j) {
      return A2($Array.get,
      A3(linearIndex,i,j,game),
      game.cells);
   });
   var neighbours = F2(function (cell,
   game) {
      return _L.fromArray([A3(getCell,
                          game,
                          cell.pos.i - 1,
                          cell.pos.j - 1)
                          ,A3(getCell,
   ...
   var flipCell = F2(function (game,
   _v10) {
      return function () {
         switch (_v10.ctor)
         {case "_Tuple2":
            return function () {
                 var cell = $Cell.flip(A3(getCell,
                 game,
                 _v10._0,
                 _v10._1));
                 return function () {
   ...
   var initFromStateParams = F2(function (stateParams,
   game) {
      return $String.isEmpty(stateParams) ? game : A3($List.foldl,
      F2(function (pair,game) {
         return function () {
            switch (pair.ctor)
            {case "Just":
               return A2(flipCell,
   ...
   var init = F2(function (_v22,
   stateParams) {
      return function () {
         switch (_v22.ctor)
         {case "_Tuple2":
            return initFromStateParams(stateParams)(preInit({ctor: "_Tuple2"
                                                            ,_0: _v22._0
                                                            ,_1: _v22._1}));}
         _U.badCase($moduleName,
         "on line 25, column 3 to 58");
      }();
   });
...
Elm.GameView.make = function (_elm) {
   ...
   var gameStateLink = function (game) {
      return $String.append("http://jafrog.com/gol-elm?")(A2($Array.foldl,
      F2(function (cell,string) {
         return A2($Basics._op["++"],
         string,
         A2($Basics._op["++"],
         ";",
         A2($Basics._op["++"],
         $Basics.toString(cell.pos.i),
         A2($Basics._op["++"],
         ",",
         $Basics.toString(cell.pos.j)))));
      }),
      "")($Array.filter(function (cell) {
         return $Cell.isAlive(cell);
      })(game.cells)));
   };
   var stateParams = function (url) {
      return function () {
         var _v0 = $List.tail(A2($String.split,
         "?",
         url));
         switch (_v0.ctor)
         {case "Just":
      ...
      return function () {
         ...
         {case "_Tuple2":
            return _U.cmp(_v4._1,
              50) < 0 ? $Maybe.Nothing : $Maybe.Just({_: {}
                                                     ,i: (_v4._1 - 50) / (model.cellSize + 1) | 0
                                                     ,j: _v4._0 / (model.cellSize + 1) | 0});}
         _U.badCase($moduleName,
   ...
   var location = Elm.Native.Port.make(_elm).inbound("location",
   "String",
   function (v) {
         ...
         {case "_Tuple2":
            return A2($Html.div,
              ...
              _L.fromArray([A2($Html.div,
                           _L.fromArray([$Html$Attributes.$class("modal")
                                        ,$Html$Attributes.id("shareDialog")
                                        ,A2($Html$Attributes.property,
                                        "aria-hidden",
                                        $Json$Encode.string("true"))]),
                           _L.fromArray([A2($Html.div,
                           _L.fromArray([$Html$Attributes.$class("modal-dialog")]),
                           _L.fromArray([A2($Html.div,
                           _L.fromArray([$Html$Attributes.$class("modal-body")]),
                           _L.fromArray([A2($Html.a,
                                        _L.fromArray([$Html$Attributes.href("#close")
                                                     ,$Html$Attributes.$class("btn-close")
                                                     ,A2($Html$Attributes.property,
                                                     "aria-hidden",
                                                     $Json$Encode.string("true"))]),
                                        _L.fromArray([$Html.text("x")]))
                                        ,A2($Html.a,
                                        _L.fromArray([$Html$Attributes.href(gameStateLink(model.game))
                                                     ,$Html$Attributes.id("share")]),
                                        _L.fromArray([$Html.text("Share me")]))]))]))]))
                           ,A2($Html.div,
                           _L.fromArray([$Html$Attributes.$class("controls")
                                        ,$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                              ,_0: "height"
                                                                              ,_1: "50px"}]))]),
                           _L.fromArray([A2($Controls.btn,
                                        "minus",
                                        mailbox.address)
                                        ,A2($Controls.btn,
                                        "play",
                           ...
                           _v23._0,
                           _v23._1)(_L.fromArray([$Graphics$Collage.move({ctor: "_Tuple2"
                                                                         ,_0: (0 - $Basics.toFloat(_v23._0)) / 2 + $Basics.toFloat(model.cellSize) / 2
                                                                         ,_1: $Basics.toFloat(_v23._1) / 2 - $Basics.toFloat(model.cellSize) / 2})(A2($Game.view,
                           model.game,
                           model.cellSize))])))]));}
         _U.badCase($moduleName,
         "between lines 83 and 128");
      }();
   });
